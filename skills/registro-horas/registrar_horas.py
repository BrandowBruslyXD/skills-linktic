...
def _seleccionar_fecha(page: Page, fecha: date) -> None:
    ...
    def _fecha_seleccionada() -> bool:
        # Validamos vía DOM (aria-selected / data-selected) en vez de innerText:
        # evita falsos negativos por timing y previene re-clicks que harían toggle.
        try:
            page.wait_for_function(
                """v => {
                    const el = document.querySelector(`[data-calendar-day][data-value="${v}"]`);
                    if (!el) return false;
                    if (el.getAttribute('aria-selected') === 'true') return true;
                    if (el.hasAttribute('data-selected')) return true;
                    return document.body.innerText.includes(window.__riporFechaEsperada || '');
                }""",
                arg=valor,
                timeout=3000,
            )
    ...
    page.evaluate("t => { window.__riporFechaEsperada = t; }", esperado)

    for intento, estrategia in enumerate(["click", "force", "dispatch"], start=1):
        if _fecha_seleccionada():
            return
        if estrategia == "click":
            locator.click()
